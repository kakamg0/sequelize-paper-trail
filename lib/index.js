...
exports.init = function (sequelize, optionsArg) {
  ...
  var beforeHook = function beforeHook(instance, opt) {
    ...
    if (options.enableCompression && !instance.isNewRecord) {
      var previousVersion = {};
      var currentVersion = {};
    ...
    if (delta && delta.length > 0) {
      ...
      instance.context.action = instance.isNewRecord ? 'create' : 'update';
  ...
  var afterHook = function afterHook(instance, opt) {
    if (debug) {
      ...
      log('opt:', opt);
      log('CLS userId:', Context.current().user);
    }

    if (Context.current().disablePaperTrail) {
      return
    }
    ...
    if (instance.context && instance.context.delta && instance.context.delta.length > 0) {
      ...
      if (failHard && !Context.current().user) {
      ...
      var revision = Revision.build({
        ...
        action: instance.context.action
      });
