...
```
2. Pass the id of the user who is responsible for a database operation to `sequelize-paper-trail` either by sequelize options or by using Context.
...
```javascript
var Context = require('sequelize-paper-trail').Context;

Context.current().user = user.id;
